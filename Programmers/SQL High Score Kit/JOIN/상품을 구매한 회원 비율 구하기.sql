...
SET @USER_COUNT = (SELECT COUNT(USER_ID) FROM USER_INFO WHERE JOINED BETWEEN '2021-01-01' AND '2021-12-31');

SELECT YEAR(T2.SALES_DATE) AS YEAR,
       MONTH(T2.SALES_DATE) AS MONTH,
       COUNT(DISTINCT T1.USER_ID) AS PURCHASED_USERS,
       ROUND(COUNT(DISTINCT T1.USER_ID) / @USER_COUNT, 1) AS PUCHASED_RATIO
